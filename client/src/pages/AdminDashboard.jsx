...
const AdminDashboard = () => {
    ...
    const fetchAllData = async () => {
        ...
        try {
            ...
            const { data: convs, error: convErr } = await api.get('api/admin/conversations');
            ...
            <div className="w-56 bg-slate-900 text-white flex flex-col p-4 sticky top-0 h-screen">
                ...
                <nav className="space-y-2 flex-1">
                    {[
                        ...
                        { id: 'sessions', label: 'All Sessions', icon: Users },
                    ].map((item) => (
                        <button
                            key={item.id}
                            ...
                            <table className="w-full text-left">
                                <thead className="bg-slate-50/50 border-b border-slate-100">
                                    {activeTab === 'counselors' ? (
                                        ...
                                        <tr>
                                            <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Student ID</th>
                                            <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Risk Level</th>
                                            <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Counsellor</th>
                                            <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Status</th>
                                            <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Date Started</th>
                                            <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Log</th>
                                        </tr>
                                    ) : (
                                        <tr>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    <AnimatePresence mode="wait">
                                        ...
                                        {activeTab === 'sessions' && conversations.map((c) => (
                                            <motion.tr key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-slate-50/50 transition-colors">
                                                <td className="px-4 py-3 font-bold text-slate-900 text-sm">{c.student_external_id}</td>
                                                <td className="px-4 py-3">
                                                    <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-tight ${c.risk_level === 'high' ? 'bg-red-50 text-red-600' : c.risk_level === 'moderate' ? 'bg-amber-50 text-amber-600' : 'bg-slate-50 text-slate-500'}`}>
                                                        {c.risk_level}
                                                    </span>
                                                </td>
                                                <td className="px-4 py-3 text-slate-500 font-medium text-xs">{c.counselors?.name || 'Unassigned'}</td>
                                                <td className="px-4 py-3">
                                                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest ${c.risk_level === 'completed' ? 'bg-green-100 text-success-700' : 'bg-blue-100 text-blue-700'}`}>
                                                        {c.risk_level === 'completed' ? 'Finished' : 'Active'}
                                                    </span>
                                                </td>
                                                <td className="px-4 py-3 text-slate-400 text-xs font-medium">{new Date(c.created_at).toLocaleDateString()}</td>
                                                <td className="px-4 py-3 text-right">
                                                    <button onClick={() => alert(`Conversation Log:\n\n${c.content || 'No messages yet.'}`)} className="text-brand-600 font-bold text-xs hover:underline">View Chat</button>
                                                </td>
                                            </motion.tr>
                                        ))}
                                    </AnimatePresence>
